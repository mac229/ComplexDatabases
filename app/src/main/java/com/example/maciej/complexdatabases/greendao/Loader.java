...
import static com.example.maciej.complexdatabases.greendao.GreenDaoProcessor.SEPARATOR;
        ...
        while ((line = tripletsSample.readLine()) != null) {
            ...
            if (songsOldIdToArtistIdMap.containsKey(oldSongId)) {
                ...
                Playback playback = new Playback(playbackCounter, songsOldIdToIdMap.get(oldSongId),
                        dateToIdMap.get(format1.format(calendar.getTime())), timeToIdMap.get(format2.format(calendar.getTime())), songsOldIdToArtistIdMap.get(oldSongId), usersOldIdToIdMap.get(oldUserId));
